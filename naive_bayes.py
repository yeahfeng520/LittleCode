...
class NaiveBayes:
	...
	def train(self, X, Y, event_model = Bernoulli, smooth = 0.5):
	    ...
	    print >> sys.stderr, 'Top 5 strong feature for each class:'
	    print >> sys.stderr, '-' * 128
...
if __name__ == '__main__':
	...
	print >> sys.stderr, 'Training accuracy for Multi-variate Bernoulli event_model : %f%%' % (100 * acc_train)
	print >> sys.stderr, 'Test accuracy for Multi-variate Bernoulli event_model : %f%%' % (100 * acc_test)

	clf.train(X_train, Y_train, Multinomial)
	acc_train = clf.test(X_train, Y_train)
	acc_test = clf.test(X_test, Y_test)

	print >> sys.stderr, 'Training accuracy for Multinomial event_model : %f%%' % (100 * acc_train)
	print >> sys.stderr, 'Test accuracy for Multinomial event_model : %f%%' % (100 * acc_test)
